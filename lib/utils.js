...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); /* eslint-disable no-param-reassign */
...
var safeEval = function safeEval(condition, scope) {
    try {
        var scopeSafe = (0, _cloneDeep2.default)(scope);
        return (0, _notevil2.default)(condition, scopeSafe);
    } catch (error) {
        return undefined;
    }
};
...
var canonicalTitleMap = function canonicalTitleMap(titleMap, originalEnum) {
    ...
    originalEnum.forEach(function (value, idx) {
        canonical.push({ name: enumValues[idx], value: value });
    });
    return canonical;
};
...
var removeEmpty = function removeEmpty(obj) {
    return Object.entries(obj).forEach(function (_ref) {
        ...
        if (val && (typeof val === "undefined" ? "undefined" : _typeof(val)) === "object") removeEmpty(val);
